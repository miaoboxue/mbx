...
<article class="page-container">
    <div class="form form-horizontal" id="form-admin-add">
        <input type="text" name="noti_id" value="{{$arr['noti_id']}}" id="noti_id">
        <div class="row cl">
            <label class="form-label col-xs-4 col-sm-3"><span class="c-red">*</span>通知公告名称：</label>
            <div class="formControls col-xs-8 col-sm-9">
                <input type="text" name="noti_name"  id="noti_name" value="{{$arr['noti_name']}}" class="input-text">
        ...
        <div class="row cl">
            ...
            <div class="formControls col-xs-8 col-sm-9">
                <input type="text" name="noti_url"  id="noti_url" value="{{$arr['noti_url']}}" class="input-text">
            </div>
        </div>
        <div class="row cl">
            ...
            <div class="formControls col-xs-8 col-sm-9">
                <dl class="permission-list">
                    <select id="new_id" name="new_id" size="1">
                        <option value="">--请选择--</option>
                        @foreach($info as $k=>$v)
                            @if($v['new_id']==$arr['new_id'])
                                <option value="{{$v['new_id']}}" selected>{{$v['new_name']}}</option>
                            @else
...
<script>
    $(function () {
        ...
        $("#btn").click(function () {
            ...
            $.post(
                '/noti/upAll',
                {noti_name:noti_name,new_id:new_id,is_status:is_status,noti_url:noti_url,noti_id:noti_id},
                function (msg) {
                    console.log(msg)
                    if (msg.status==505){
                        alert('通知公告名称不能为空')
