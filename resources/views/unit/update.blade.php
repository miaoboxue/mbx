...
<article class="page-container">
    <div class="form form-horizontal" id="form-admin-add">
        <input type="hidden" name="unit_id" value="{{$arr['unit_id']}}" id="unit_id">
        <div class="row cl">
            <label class="form-label col-xs-4 col-sm-3"><span class="c-red">*</span>直属单位信息名称：</label>
            <div class="formControls col-xs-8 col-sm-9">
                <input type="text" name="unit_name"  id="unit_name" value="{{$arr['unit_name']}}" class="input-text">
        ...
        <div class="row cl">
            ...
            <div class="formControls col-xs-8 col-sm-9">
                <textarea name="unit_text" id="unit_text" cols="30" rows="10">{{$arr['unit_text']}}</textarea>
                ...
                <dl class="permission-list">
                    <select id="vk_id" name="vk_id" size="1">
                        ...
                        <?php foreach($info as $v){ ?>
                            @if($v['vk_id']==$arr['vk_id'])
                                <option value="{{$v['vk_id']}}" selected>{{$v['vk_name']}}</option>
                            @else
...
<script>
    $(function () {
        ...
        $("#btn").click(function () {
            ...
            $.post(
                '/unit/updateall',
                {unit_name:unit_name,unit_text:unit_text,vk_id:vk_id,is_status:is_status,unit_id:unit_id},
                function (msg) {
                    console.log(msg)
                    if(msg.status=='1500'){
                        alert('直属单位名称不能为空')
