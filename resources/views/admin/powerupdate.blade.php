    ...
    <div class="form form-horizontal" id="form-admin-role-add">
        <div class="row cl">
            ...
            <div class="formControls col-xs-8 col-sm-9">
                <input type="text" class="input-text" value="{{$data['action_name']}}" readonly  name="action_name">
            </div>
        </div>
        <div class="row cl">
            <label class="form-label col-xs-4 col-sm-3"><span class="c-red">*</span>路由方法：</label>
            <div class="formControls col-xs-8 col-sm-9">
                <input type="text" class="input-text" value="{{$data['action_url']}}"  name="action_url">
            </div>
        </div>
        <div class="row cl">
            ...
            <div class="formControls col-xs-8 col-sm-9">
                <dl class="permission-list">
                    <select class="pid" name="pid" size="1">
                        <option value="0">--请选择--</option>
                        @foreach($powerInfo as $k=>$v)
                            @if($v['action_id']==$data['action_id'])
                                <option value="{{$v['action_id']}}" selected>{{$v['action_name']}}</option>
                            @else
                                <option value="{{$v['action_id']}}">{{$v['action_name']}}</option>
                            @endif
                        @endforeach
                    </select>
                </dl>
    ...
    $(function(){
        ...
        $('.btn').click(function(){
            ...
            $.ajax({
                url:"/poweradd/do",
                type:"post",
                data:{action_name:action_name,action_url:action_url,pid:pid,is_show:is_show},
                dataType:"json",
